    ...
    selection.each(function (map_idx_obj) {
           ...
           // cards.select("title").text(function(d) { return d.scanstep_difference; });
            ...
            cards.on("click", function(d) {
                //highlight_clicked_card_from_compare(d);	//from d3js_695c_02.js
                if (d.query == query_ls.indexOf("Q_sum") && d.table == table_ls.indexOf("T_sum")) 
                {
                	d3.select("#detail")
            		  .append("div")
            		  .text("Sum of scansteps for all queries and all tables");
                }
                else if (d.table == table_ls.indexOf("T_sum") + 1)	//table start from 1
                {
		            var query_detail = query_text[d.query];
		            var query_arr = [];
		            query_arr.push(query_detail);
		            var detail_div = d3.select("#detail")
		            				   .selectAll("div")
		            				   .data(query_detail)
		            				   .enter()
		            				   .append("div")
		        ...
		        else if (d.query == query_ls.indexOf("Q_sum"))
		        {
		        	var table_detail = table_text[d.table];
		        	var detail_div = d3.select("#detail")
		            				   .selectAll("div")
		            				   .data(table_detail)
		            				   .enter()
		        ...
		        else
		        {
		        	d3.select("#detail")
            		  .append("div")
            		  .text("Click on summary cells for details");
		        }
            });
...
function load_all_query_and_table_text()
{
	d3.tsv("query_text.tsv", function(error, data) {
        //query_text[d.number] = d.text;
	...
	d3.tsv("table_text.tsv", function(error, data) {
        //query_text[d.number] = d.text;
	 	...
	 	var node = svg.selectAll(".node")
	  	...
	  	node.append("text")
		  	    ...
		  	    .on("click", function(d) {
		  	    	...
		  	    	/*d3.select("#heatmap")
		  	    	  .selectAll("div").remove();*/	//don't remove last avg state
		  	    	
		  	    	//draw_avg_state(data, data.length);
		  	    	draw_avg_only(data, data.length);
		  	    });
    });
...
function draw_avg_only(subset, cnt)  //subset[cnt] is an array of filenames
{
	var url = "/comp_avg/";
	for (var i = 0; i < cnt; ++i)
	{
	...
	d3.tsv(url, function(d) {
		...
		if (error === null) {
			//debugger;
			subset = [];	//clear the leaf nodes in subset
			datasets_table["avg.tsv"] = d;
			subset.unshift(datasets.indexOf("avg.tsv"));
			/*d3.select("#heatmap")
  	    	  .selectAll("div").remove();*/	//don't remove last avg state
...
function draw_collapsible_clustering_tree() {
	...
	d3.json("cluster.json", function(error, flare) {
	  if (error) throw error;
